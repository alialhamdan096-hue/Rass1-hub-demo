...
document.getElementById('summaryTb').innerHTML = 
    ...
    '<tr><td>⏳ Waiting</td><td>' + w + '</td><td>' + wr + '%</td></tr>';

document.getElementById('convList').innerHTML = cp.length > 0 
    ? cp.map(p => '<tr><td class="name">' + Utils.sanitize(p.name) + '</td><td class="phone">' + Utils.sanitize(p.phone) + '</td><td class="med">' + Utils.sanitize(p.med) + '</td><td>' + p.reminderDate + '</td><td>' + p.convertedDate + '</td></tr>').join('') 
    : '<tr><td colspan="5"><div class="empty">No conversions</div></td></tr>';
```
...
// Branch summary
const br = Object.entries(bs)
    .sort((a, b) => b[1].total - a[1].total)
    .map(([b, s]) => '<tr><td><strong>' + b + '</strong></td><td>' + s.total + '</td><td>' + s.delivered + '</td><td>' + (s.total - s.delivered) + '</td></tr>')
    .join('');
document.getElementById('branchSummary').innerHTML = br || '<tr><td colspan="4"><div class="empty">No orders</div></td></tr>';

// Top items
const tm = Object.entries(mc)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
    .map(([med, cnt]) => '<tr><td class="med">' + Utils.sanitize(med) + '</td><td>' + cnt + '</td></tr>')
    .join('');
